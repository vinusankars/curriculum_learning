...
def balance_order(order, dataset):
    ...
    size_each_class = min([len(i) for i in class_orders])
    ...
    if len(new_order) == len(dataset.x_train):
        return new_order

    group = []
    for cls in range(num_classes):
        group += class_orders[cls][size_each_class:]
    for idx in group:
        new_order.append(order[idx])
...
def run_expriment(args):
    ...
    try:
        ...
        if output_path:
            with open(output_path + "_interrupt"+str(repeat)+"_history", 'wb') as file_pi:
